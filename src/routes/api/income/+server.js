...
export const GET = ({ url }) => {
	...
	const creditSource = url.searchParams.get('creditSource');
	...
	let query = `
		SELECT
			...
			e.entity_type,
			(
				SELECT GROUP_CONCAT(ip.project_id)
				FROM income_projects ip
				WHERE ip.income_id = i.id
			) as project_ids_csv,
			(
				SELECT GROUP_CONCAT(p2.property_name, ', ')
				FROM income_projects ip2
				JOIN projects p2 ON p2.id = ip2.project_id
				WHERE ip2.income_id = i.id
			) as project_names,
			a.account_holder,
			a.bank_name
		FROM income i
	...
	query += start && end ? ' ORDER BY i.id DESC' : ' ORDER BY i.created_at DESC, i.id DESC';
	...
	const filteredRows = rows.filter((row) => {
		...
		if (creditSource && creditSource !== 'all' && String(row.credit_source || '') !== creditSource) return false;
		if (
			projectId &&
			projectId !== 'all' &&
...
export const POST = async ({ request }) => {
	...
	const tx = db.transaction(() => {
		...
		const result = db
			.prepare(
				`INSERT INTO income
				(date, amount, entity_id, project_id, account_id, payment_mode, credit_source, master_owner, comments)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
			)
			.run(
				date,
	...
	return json(db.prepare('SELECT * FROM income WHERE id = ?').get(insertedId), { status: 201 });
};
