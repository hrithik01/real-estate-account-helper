<script>
	...
	const sanitizeExpenseTypeFields = (payload) => ({
		...payload,
		material_type: /material|service|utility/i.test(payload.expense_source || '') ? payload.material_type : '',
		service_type: /material|service|utility/i.test(payload.expense_source || '') ? payload.service_type : '',
		utility_type: /material|service|utility/i.test(payload.expense_source || '') ? payload.utility_type : ''
	});
	...
	const loadMasters = async () => {
		...
		if (optionsRes.ok) {
			...
			expenseSources = options.expense_sources || [];
			if (!expenseSources.includes(form.expense_source)) {
				form.expense_source = expenseSources[0] || 'other';
			}
		}
	...
	const submitExpense = async () => {
		...
		const payload = sanitizeExpenseTypeFields({
			...form,
		...
		if (res.ok) {
			...
			const selectedFormDate = form.date || getPreferredFormDate(expenseList);
			form = {
...
<section class="panel">
	<div class="row">
		<div>
			<h2>Expense Entry</h2>
			<p class="muted">Track outgoing transactions with linked entity/project/account.</p>
		</div>
	</div>
	...
	<div class="form-grid">
		...
		<label><span>Entity *</span><select bind:value={form.entity_id}><option value="">Select</option>{#each availableFormEntities as entity}<option value={entity.id}>{entity.name} ({entity.entity_code})</option>{/each}</select></label>
		<label><span>Projects (Optional, multiple)</span>
		...
		<label><span>Account</span><select bind:value={form.account_id} disabled={form.payment_mode !== 'account'}><option value="">Select</option>{#each accounts as account}<option value={account.id}>{account.account_holder} ({account.bank_name})</option>{/each}</select></label>
		<label><span>Transaction Type</span><select bind:value={form.payment_mode}>{#each PAYMENT_MODES as mode}<option value={mode}>{mode}</option>{/each}</select></label>
		<label><span>Expense Source</span><select bind:value={form.expense_source}>{#each expenseSources as source}<option value={source}>{source}</option>{/each}</select></label>
		{#if /material|service|utility/i.test(form.expense_source || '')}
			<label><span>Material Type</span><select bind:value={form.material_type}><option value="">N/A</option>{#each MATERIAL_TYPE as item}<option value={item}>{item}</option>{/each}</select></label>
			<label><span>Service Type</span><select bind:value={form.service_type}><option value="">N/A</option>{#each SERVICE_TYPE as item}<option value={item}>{item}</option>{/each}</select></label>
			<label><span>Utility Type</span><select bind:value={form.utility_type}><option value="">N/A</option>{#each UTILITY_TYPE as item}<option value={item}>{item}</option>{/each}</select></label>
		{/if}
		<label><span>Attachment (HKJ / PKB / SHREE)</span><select bind:value={form.master_owner}>{#each MASTER_OWNERS as owner}<option value={owner.key}>{owner.label}</option>{/each}</select></label>
	...
	<table>
		...
		<tbody>
			{#if filteredExpenseList.length === 0}
				<tr><td colspan={5 + (showProjects ? 1 : 0) + (showType ? 1 : 0) + (showSource ? 1 : 0) + (showAttachment ? 1 : 0) + (showAccount ? 1 : 0) + (showTimestamp ? 1 : 0)} class="muted">No expense entries.</td></tr>
			{:else}
				{#each filteredExpenseList as row}
					<tr>
						...
						<td>{#if editingId === row.id}<select bind:value={draft.entity_id}><option value="">Select</option>{#each availableDraftEntities as entity}<option value={entity.id}>{entity.name}</option>{/each}</select>{:else}{row.entity_name || '-'}{/if}</td>
						{#if showProjects}
							<td>
								{#if editingId === row.id}
									<div class="multi-select">
										<button type="button" class="multi-trigger" on:click={() => (draftProjectsOpen = !draftProjectsOpen)}>{getSelectedProjectNames(draft.project_ids)}</button>
										{#if draftProjectsOpen}
											<div class="multi-menu">
												{#each projects as project}
													<label class="multi-option">
															<input type="checkbox" checked={draft.project_ids?.map(String).includes(String(project.id))} on:change={() => updateDraftProjectSelection(project.id)} />
														<span>{project.property_name}</span>
													</label>
												{/each}
											</div>
						...
						{/if}
						{#if showType}<td>{#if editingId === row.id}<select bind:value={draft.payment_mode}>{#each PAYMENT_MODES as mode}<option value={mode}>{mode}</option>{/each}</select>{:else}{row.payment_mode}{/if}</td>{/if}
						...
						<td>
							...
							{:else}
								...
								<button class="ghost" on:click={() => deleteExpense(row.id)}>Delete</button>
							{/if}
						</td>
					</tr>
				{/each}
